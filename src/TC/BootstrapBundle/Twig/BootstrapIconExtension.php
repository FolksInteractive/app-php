...
namespace TC\BootstrapBundle\Twig;
...
class BootstrapIconExtension extends Twig_Extension
{
    ...
    public function parseIconsFilter($text)
    {
        ...
        if( 0 === strpos( $text, ".glyphicon" )){
            return preg_replace_callback(
                '/\.glyphicon-([a-z0-9-]+)/',
                function ($matches) use ($that) {
                    return $that->glyphiconFunction($matches[1]);
                },
                $text
            );
        }
    ...
    /**
     * Returns the HTML code for the given glyphicon.
     *
     * @param string $icon  The name of the glyphicon
     *
     * @return string The HTML code for the glyphicon
     */
